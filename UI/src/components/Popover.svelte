<script>
  ...
  function getHourClass(coverage) {
    switch (coverage) {
      case '0/3':
        return 'hour-0-coverage';
      case '1/3':
        return 'hour-1-coverage';
      case '2/3':
        return 'hour-2-coverage';
      case '3/3':
        return 'hour-3-coverage';
      default:
        return '';
    }
  }
...
<style>
  ...
  .hour-1-coverage {
    background-color: #fab75f; /* orange */
  }
    ...
    <div class="scrollable">
      {#each Array(24) as _, i}
        <div class="hour {getHourClass(hourlyCoverage[i]?.status)}">
          ...
          <span class="hour-status">{hourlyCoverage[i]?.status || '0/3'}</span>
